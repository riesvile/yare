...
function end_game(was_p1 = 0, was_p2 = 0) {
  ...
  var compressed_file = zlib
    .deflateSync(JSON.stringify(game_file))
    .toString("base64");
          ...
          .then((result_p1) => {
            ...
            User.find({ user_id: players["p2"] })
              .then((result_p2) => {
                ...
                Game.updateOne(
                  { game_id: workerData[0] },
                  { p1_rating: result_p1[0]["rating"], p2_rating: p222_rating },
                  { upsert: true }
                ).then((qq) => {
...
if (!isMainThread) {
  ...
  function progress_tut(phase_done, log = false) {
    if (log) console.log("tutorial phase " + phase_done + " done");
    let i = phase_done - 1;
  ...
  function process_stuff() {
    ...
    if (
      base_lookup["base_" + players["p1"]].energy >=
      base_lookup["base_" + players["p1"]].current_spirit_cost
    ) {
      if (workerData[1] == "tutorial" && top_s > 20) {
        ...
        if (p1_defend != 1) {
          ...
          if (workerData[1] == "tutorial") progress_tut(5, true);
        }
      }
    }
    if (
      base_lookup["base_" + players["p2"]].energy >=
      base_lookup["base_" + players["p2"]].current_spirit_cost
    ) {
      if (p2_defend != 1) {
        top_q++;
  ...
  async function update_state() {
    ...
    if (game_duration >= 0) {
      ...
      if (render_data3.g1.length > gqueue_cutoff) {
        ...
        log1.push(
          "WARN: graphics output too long (>" +
            gqueue_cutoff +
            " commands), cutting off " +
            (l1 - gqueue_cutoff) +
            " commands"
        );
      }
  ...
  function game_start() {
    ...
    global["base_" + players["p1"]] = new Base(
      "base_" + players["p1"],
      [1600, 700],
      players["p1"],
      colors["player1"],
    ...
    global["base_" + players["p2"]] = new Base(
      "base_" + players["p2"],
      [2600, 1700],
      players["p2"],
      colors["player2"],
