...
function getNewRating(playerRating, opponentRating, playerResult) {
  return (
    playerRating + getRatingDelta(playerRating, opponentRating, playerResult)
  );
}
      ...
      Game.find({ game_id: workerData[0] })
        .then(async (result) => {
          ...
          if (result[0]["ranked"] == 0) {
            Game.updateOne(
              { game_id: workerData[0] },
              { active: 0, winner: gameWinner, game_history: game_history },
              { upsert: true }
            ).then((qq) => {
...
parentPort.on("message", (message) => {
  logger = pino({
    transport: {
      targets: [
        ...
        {
          target: "pino/file",
          options: { destination: `/var/log/game-${workerData[0]}.log` },
          level: "trace",
        },
      ],
    },
  ...
  if (message.data == "initiate world") {
    ...
    parentPort.postMessage({
      data: JSON.stringify({ meta: "initiate", data: init_data }),
      game_id: workerData[0],
      meta: "initiate",
    ...
    if (message.pl_num == "player1") {
      if (
        message.session_id == player1_session ||
        message.pl_id == "anonymous"
      ) {
        ...
        Session.findOne({ session_id: message.session_id })
          .then((session) => {
            if (session) {
      ...
      .then((result) => {
        ...
        User.find({ user_id: { $in: [players["p1"], players["p2"]] } })
          .then((results) => {
            ...
            Game.updateOne({ game_id: workerData[0] }, updates, {
              upsert: true,
            }).then((qq) => {
  ...
  if (sourcemap !== null) {
    stack = await Promise.all(
      stack.map(async (l) => {
        ...
        let base64SourceMap = sourcemap.split(
          /^data:(application|text)\/json;base64,/
        )[2];

        let textSourceMap = Buffer.from(base64SourceMap, "base64").toString(
          "ascii"
        );

        let rawSourceMap = JSON.parse(textSourceMap);

        let originalPosition = await SourceMapConsumer.with(
          rawSourceMap,
          null,
          (consumer) => {
...
function jump_danger_zone(loc) {
  if (
    (Math.abs(stars[0].position[0] - loc[0]) < 100 &&
      Math.abs(stars[0].position[1] - loc[1]) < 100) ||
    (Math.abs(stars[1].position[0] - loc[0]) < 100 &&
      Math.abs(stars[1].position[1] - loc[1]) < 100) ||
    (Math.abs(stars[2].position[0] - loc[0]) < 100 &&
      Math.abs(stars[2].position[1] - loc[1]) < 100) ||
    (Math.abs(stars[3].position[0] - loc[0]) < 100 &&
      Math.abs(stars[3].position[1] - loc[1]) < 100) ||
    (Math.abs(bases[0].position[0] - loc[0]) < 50 &&
      Math.abs(bases[0].position[1] - loc[1]) < 50) ||
    (Math.abs(bases[1].position[0] - loc[0]) < 50 &&
      Math.abs(bases[1].position[1] - loc[1]) < 50) ||
    (Math.abs(bases[2].position[0] - loc[0]) < 50 &&
      Math.abs(bases[2].position[1] - loc[1]) < 50) ||
    (Math.abs(bases[3].position[0] - loc[0]) < 50 &&
      Math.abs(bases[3].position[1] - loc[1]) < 50) ||
    (Math.abs(outposts[0].position[0] - loc[0]) < 50 &&
      Math.abs(outposts[0].position[1] - loc[1]) < 50) ||
    (Math.abs(pylons[0].position[0] - loc[0]) < 50 &&
      Math.abs(pylons[0].position[1] - loc[1]) < 50)
  ) {
    return true;
  } else {
    return false;
...
var sandboxCode = fs.readFileSync(__dirname + "/sandbox.js", {
  encoding: "utf8",
});
...
class Sandbox {
  constructor() {
    ...
    this.yd = this.context.evalClosureSync(sandboxCode, [], {
  ...
  loadAddons(names) {
    for (let name of names) {
      this.context.evalClosureSync(addons.get(name), [], {
        result: { reference: true },
      });
    }
  }
  ...
  channel(name, data) {
    try {
      this.funcs.channel_in.applySync(this.yd.derefInto(), [name, data], {
        arguments: { copy: true },
      });
    } catch (e) {
      logger.error(e);
  ...
  setPlayerCode(code) {
    ...
    try {
      ...
      this.last_compile_err =
        "Submitted code has a compile error:\n    " + err.message;
  ...
  init(player_id) {
    this.yd
      .getSync("init", { reference: true })
      .applySync(this.yd.derefInto(), [player_id], {
        arguments: { copy: true },
      });
  }

  async loadData() {
    this.funcs.loadData.apply(
      this.yd.derefInto(),
      [
        {
          tick: ticks["now"],
          ...
          spirits: rawSpirits,
          fragments: fragments,
          stars: JSON.parse(JSON.stringify(star_lookup)),
          bases: JSON.parse(JSON.stringify(base_lookup)),
          outposts: JSON.parse(JSON.stringify(outpost_lookup)),
          pylons: JSON.parse(JSON.stringify(pylon_lookup)),
          players: JSON.parse(JSON.stringify(players)),
        },
      ],
      { arguments: { copy: true }, result: { reference: true } }
    );
  }
  ...
  async run() {
    ...
    logger.debug(
      "sandbox run in " + ((post - pre) / 1000000n).toString() + " ms"
    );
  }

  async output() {
    return this.funcs.getOutput.apply(this.yd.derefInto(), [], {
      result: { copy: true },
    });
  }
}
...
if (!isMainThread) {
  ...
  function fast_dist_leq(item1, item2, range) {
    return (
      (item2[0] - item1[0]) ** 2 + (item2[1] - item1[1]) ** 2 <= range ** 2
    );
  }

  function fast_dist_simp(item1, item2, range) {
    return (
      Math.abs(item1[0] - item2[0]) <= range &&
      Math.abs(item1[1] - item2[1]) <= range
    );
  }
      ...
      for (s = 3; s < 7 * 4 - 1; s++) {
        ...
        for (let i = 1; i < bin.length; i++) {
          for (let j = 1; j < nb.length; j++) {
            ...
            if (dsq <= visible_sq) work(bin[i], nb[j]);
            if (dsq <= beamable_sq) work_beamable(bin[i], nb[j]);
          }
        }
      }
    });
  ...
  function progress_tut(phase_done, log = false) {
    if (log) logger.debug("tutorial phase " + phase_done + " done");
    let i = phase_done - 1;
    ...
    for (let player in all_commands) {
      ...
      Object.keys(queue).forEach((id) => {
        ...
        if (!id || !player_owns_spirit(id, player)) {
          logger.info(
            "WTF: null or possible hack: player " +
              player +
              " calls " +
              id +
              ".move()"
        ...
        if (len_sq > 0) {
          ...
          for (let k = 0; k < potential_structure_collisions.length; k++) {
            ...
            if (fast_dist_lt(spirit.position, object_position, min_distance)) {
              ...
              let pick_first =
                quick_dist1 < quick_dist2 ||
                Math.abs(quick_dist1 - quick_dist2) <= 5;
  ...
  function energize_objects() {
    ...
    for (let player in all_commands) {
      ...
      for (let spirit in commands) {
        ...
        if (!(spirit in spirit_lookup) || spirit_lookup[spirit].hp == 0)
          continue;
        ...
        for (let j = 0; j < explodee.sight.enemies_beamable.length; j++) {
          let potential_target =
            spirit_lookup[explodee.sight.enemies_beamable[j]];
          //logger.debug('boom check = ' + fast_dist_lt(explodee.position, potential_target.position, 100));
          if (
            fast_dist_leq(explodee.position, potential_target.position, 160)
          ) {
    ...
    for (let i = 0; i < outposts.length; i++) {
      ...
      if (enemies.length == 0 || outpost.control == "") continue;

      let beam_strength = outpost.energy >= 500 ? 4 : 1;
      let enemy =
        spirit_lookup[enemies[Math.floor(enemies.length * Math.random())]];
    ...
    for (let p = 0; p < pylons.length; p++) {
      ...
      let friends_damaged = [];
      let friends_final = [];
      if (friends.length == 0 || pylon.control == "") continue;

      for (let f = 0; f < friends.length; f++) {
        let friend_real = spirit_lookup[friends[f]];
        if (friend_real.energy < friend_real.energy_capacity) {
          if (dist_sq(friend_real.position, pylon.position) > low_range_sq)
            friends_final.push(friend_real);
        }
      }
    ...
    for (let player in all_commands) {
      ...
      Object.keys(queue).forEach((from_id) => {
        ...
        if (last_beam[from_id] != undefined) return;
        ...
        if (from_id == to_id) {
          ...
          for (let f = 0; f < from_obj.sight.fragments.length; f++) {
            ...
            let fragment_close = fast_dist_leq(
              from_obj.position,
              fragment.position,
              from_obj.range
            );
            ...
            energize_apply_fragment.push([
              from_obj,
              energy_value * from_obj.size,
              fragment,
            ]);
            is_harving = 1;
          }
          ...
          for (let j = 0; j < from_obj.sight.structures.length; j++) {
            ...
            // name prefix - safe (is structure)
            if (!struc_name.startsWith("star")) continue;

            let star = structure_lookup[struc_name];
            let star_close = fast_dist_leq(
              from_obj.position,
              star.position,
              from_obj.range
            );
        ...
        let target_close = fast_dist_leq(
          from_obj.position,
          to_check,
          from_obj.range
        );
        ...
        let beam_strength = Math.min(
          energy_value * from_obj.size,
          from_obj.energy
        );
        if (beam_strength <= 0) return;

        from_obj.last_energized = to_id;

        let friendly_beam = from_obj.player_id == to_obj.player_id;
        let is_star =
          to_obj.structure_type != undefined && to_obj.structure_type == "star";
    ...
    for (let i = energize_apply_fragment.length - 1; i >= 0; i--) {
      ...
      let to_full_capacity = Math.max(
        0,
        spirit.energy_capacity - spirit.energy
      );
    ...
    for (let i = 0; i < outposts.length; i++) {
      ...
      if (outpost.energy <= 0) outpost.control = "";
      outpost.energy = Math.max(
        0,
        Math.min(outpost.energy, outpost.energy_capacity)
      );
    ...
    for (let i = energize_apply_pylon.length - 1; i >= 0; i--) {
      ...
      if (spirit.player_id == players["p1"]) {
        if (incoming_p1[pylon.id] == undefined) incoming_p1[pylon.id] = 0;
        incoming_p1[pylon.id] += amount;
        //logger.debug('incoming amount 1 = ' + incoming_p1[pylon.id]);
      } else {
        if (incoming_p2[pylon.id] == undefined) incoming_p2[pylon.id] = 0;
    ...
    for (let i = 0; i < pylons.length; i++) {
      ...
      if (pylon.control == "") {
        // the case where from_p1 == from_p2 will have 0 energy and control '' set below
        pylon.control =
          from_p1_pylon > from_p2_pylon ? players["p1"] : players["p2"];
        pylon.energy = Math.abs(from_p1_pylon - from_p2_pylon);
      } else {
        let from_me_pylon =
          pylon.control == players["p1"] ? from_p1_pylon : from_p2_pylon;
        let from_enemy_pylon =
          pylon.control == players["p1"] ? from_p2_pylon : from_p1_pylon;

        pylon.energy += from_me_pylon;
        pylon.energy -= 2 * from_enemy_pylon;
      }

      if (pylon.energy <= 0) pylon.control = "";
    ...
    for (let i = 0; i < bases.length; i++) {
      ...
      if (base.energy < 0) {
        ...
        base.shape = "neutral";
        logger.debug(
          "find out whether player controls any other structures - otherwise end the game"
        );
  ...
  function process_stuff() {
    ...
    for (i = 0; i < bases.length; i++) {
      ...
      if (bs.control == players["p1"]) {
        top_s++;
        global[players["p1"] + top_s] = new Spirit(
          players["p1"] + "_" + top_s,
          [bs.position[0] + x_axis * 40, bs.position[1] + y_axis * 40],
          get_def_size(shapes["player1"]),
          get_def_size(shapes["player1"]) * 10,
          players["p1"],
          colors["player1"],
          shapes["player1"]
        );
      ...
      if (bs.control == players["p2"]) {
        top_q++;
        global[players["p2"] + top_q] = new Spirit(
          players["p2"] + "_" + top_q,
          [bs.position[0] + x_axis * 40, bs.position[1] + y_axis * 40],
          get_def_size(shapes["player2"]),
          get_def_size(shapes["player2"]) * 10,
          players["p2"],
          colors["player2"],
          shapes["player2"]
        );
    ...
    for (let player in all_commands) {
      ...
      for (let spirit in commands) {
        ...
        if (!(spirit in spirit_lookup) || spirit_lookup[spirit].hp == 0)
          continue;
        if (commands[spirit].shout)
          render_data3.s.push(["sh", spirit, commands[spirit].shout]);
        if (commands[spirit].mark)
          spirit_lookup[spirit].mark = commands[spirit].mark;
      }
    }
    //
    // objects energize
    ...
    for (let i = 0; i < stars.length; i++) {
      ...
      if (stars[i].energy >= stars[i].energy_capacity)
        stars[i].energy = stars[i].energy_capacity;
    ...
    for (let i = death_queue.length - 1; i >= 0; i--) {
      ...
      if (workerData[1] == "tutorial") {
        if (death_queue[i].id == "easy-bot_2") progress_tut(8, true);
      }
    ...
    for (let player in all_commands) {
      ...
      for (let spirit in commands) {
        ...
        console.log(
          "jump distanceEdistanceEdistanceEdistanceEdistanceEdistanceEdistanceEdistanceEdistanceE = " +
            dist
        );
        let cost = dist / 4 + s.size ** 2 / 4;
        console.log(
          "cost distanceEdistanceEdistanceEdistanceEdistanceEdistanceEdistanceEdistanceEdistanceE = " +
            cost
        );
        if (cost > s.energy) {
    ...
    if (temp_flag == 0) {
      ...
      for (i = 0; i < living_spirits.length; i++) {
        ...
        if (spt.player_id == players["p2"]) {
          //render3 part
          render_data3.p2.push([
            cutoff_id,
            [
              Math.round(spt.position[0] * 100) / 100,
              Math.round(spt.position[1] * 100) / 100,
            ],
            spt.size,
            spt.energy,
            spt.hp,
          ...
          if (spt.hp == 1) {
            p2_living++;
            if (spt.shape == "circles" && spt.size > 1)
              p2_living += spt.size - 1;
          }
        } else if (spt.player_id == players["p1"]) {
          //render3 part
          render_data3.p1.push([
            cutoff_id,
            [
              Math.round(spt.position[0] * 100) / 100,
              Math.round(spt.position[1] * 100) / 100,
  ...
  async function update_state() {
    ...
    if (
      game_duration < -3 &&
      sand1.successful_compile &&
      sand2.successful_compile
    ) {
      game_duration = -3;
    }
    ...
    render_data3.b1 = [
      bases[0].energy,
      bases[0].current_spirit_cost,
      bases[0].sight.enemies.length,
      bases[0].control,
    ];
    render_data3.b2 = [
      bases[1].energy,
      bases[1].current_spirit_cost,
      bases[1].sight.enemies.length,
      bases[1].control,
    ];
    render_data3.b3 = [
      bases[2].energy,
      bases[2].current_spirit_cost,
      bases[2].sight.enemies.length,
      bases[2].control,
    ];
    render_data3.b4 = [
      bases[3].energy,
      bases[3].current_spirit_cost,
      bases[3].sight.enemies.length,
      bases[3].control,
    ];
    ...
    parentPort.postMessage({
      data: JSON.stringify(render_data3),
      user_data: user_data,
      game_id: workerData[0],
      meta: "",
    });
  ...
  function game_start() {
    ...
    for (s = 1; s < 1 + start_num_spirits - start_num_adjust1; s++) {
      if (s > 6) {
        global[players["p1"] + s] = new Spirit(
          players["p1"] + "_" + s,
          [-620 - s * 20, -600],
          get_def_size(shapes["player1"]),
          get_def_size(shapes["player1"]) * 10,
    ...
    for (q = 1; q < 1 + start_num_spirits - start_num_adjust2; q++) {
      if (q > 6) {
        global[players["p2"] + q] = new Spirit(
          players["p2"] + "_" + q,
          [450 + q * 20, 770],
          get_def_size(shapes["player2"]),
          get_def_size(shapes["player2"]) * 10,
