...
const logger = pino({
  transport: {
    targets: [
      ...
      {
        target: "pino/file",
        options: { destination: `/var/log/game-server-${this_server}.log` },
        level: "trace",
      },
    ],
  },
...
function create_worker(game_id, game_type) {
  const worker = new Worker(base + "/game/game.js", {
    workerData: [game_id, game_type],
  });
  worker.on("error", (err) => {
    throw err;
  });
...
function start_world(game_id) {
  //starts the game
  try {
    workers[game_id].postMessage({
      data: "start world",
      player1: active_games[game_id][1],
      player2: active_games[game_id][2],
      p1_shape: active_games[game_id][4],
      p2_shape: active_games[game_id][5],
      p1_color: active_games[game_id][6],
      p2_color: active_games[game_id][7],
    });
  } catch (error) {
    logger.error(error);
  }
  ...
  wss.clients.forEach(function each(client) {
    ...
    if (client.readyState === WebSocket.OPEN) {
      if (client.game_id == game_id) {
        ...
        client.send(
          JSON.stringify({ render: render, chan: userData[user_id] })
        );
      }
    }
  });
...
wss.on("connection", function connection(ws, req) {
  ...
  ws.on("message", async function incoming(message) {
    ...
    if (message.meta == "channel") {
      workers[g_id].postMessage({
        data: "channel",
        user_id: ws.user_id,
        channel: message.channel,
        chan_data: message.data,
      });
      return;
    }

    if (
      message["u_code_lang"] != undefined &&
      message["u_code_lang"] != "javascript"
    ) {
      let req = await fetch(config.frontendAddress + "/transpiler/transpile", {
        method: "POST",
        body: JSON.stringify({
          code: message["u_code"],
          language: message["u_code_lang"],
        }),
...
app.use(
  "/" + this_server + "/a/",
  express.static(base + "/public", { extensions: ["html"] })
);
