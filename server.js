...
const sha256 = (hash_string) =>
  crypto.createHash("sha256").update(hash_string, "utf8").digest("hex");
...
function pick_server(type) {
  ...
  let total_count =
    Object.keys(type_servers).reduce((a, s) => a + (server_count[s] || 0), 0) +
    1;
      ...
      User.find({ user_id: req.body.user_id })
        .then((result) => {
          ...
          if (req.body.user_id == "anonymous") {
            ...
            automatch_looking.push([
              result[0].user_id,
              result[0].rating,
              req.body.user_shape,
              get_color(req.body.user_color),
              0,
              0,
            ]);
            res.status(200).send({
    ...
    .then((result) => {
      ...
      if (result.length == 0) {
        ...
        if (good) {
          ...
          var sessionCreatePromise = Session.create({
            user_id: user_id,
            session_id: session_id,
            session_expire: session_expire,
          });

          Promise.all([updatePromise, sessionCreatePromise]).then(() => {
            res.status(200).send({
              user_id: user_id,
              data: session_id,
            });
  ...
  Game.find({ game_id: req.body.game_id })
    .then((result) => {
      ...
      if (result.length == 0) {
        ...
        workers[req.body.game_id].postMessage({
          data: "update anonymous",
          player1: req.body.user_id,
        });
      } else {
        res.status(404).send({
...
setInterval(function () {
  ...
  Game.find({ game_id: { $in: game_ids } })
    .then((games) => {
      ...
      if (updates.length > 0) {
        Game.updateMany({ game_id: { $in: updates } }, { active: 0 }).catch(
          (err) => {
            console.log(err);
          }
        );
  ...
  Game.find({ game_id: req.body.game_id })
    .then((result) => {
      ...
      if (result.length == 0) {
        ...
        var decompressed_file = zlib
          .inflateSync(Buffer.from(result[0].game_file, "base64"))
          .toString();
...
app.post("/populate-hub", (req, res) => {
  Game.find({
    $or: [{ player1: req.body.user_id }, { player2: req.body.user_id }],
  })
    .sort({ updatedAt: "desc" })
    .limit(10)
    .exec()
  ...
  Game.find({ game_id: g_id })
    .then((result) => {
      ...
      if (result.length == 0) {
        ...
        Game.updateOne(
          { game_id: g_id },
          { player2: req.body.user_id, p2_session_id: req.body.session_id },
          { upsert: true }
        ).then((qq) => {
          ...
          if (active_game != undefined) {
            ...
            console.log(
              "WTF game " +
                g_id +
                " probably canceled in the meantime? race condition?"
            );
          }
        });
      ...
      .then((result) => {
        ...
        if (result.length == 0) {
          ...
          Game.updateOne(
            { game_id: g_id },
            {
              player2: req.body.user_id,
              p2_session_id: req.body.session_id,
              p2_shape: req.body.user_shape,
              p2_color: get_color(req.body.user_color),
            },
            { upsert: true }
          ).then((qq) => {
            let active_game = active_games[g_id];
  ...
  Game.find({ game_id: g_id })
    .then((result) => {
      ...
      if (result.length == 0) {
        ...
        Game.updateOne({ game_id: g_id }, { active: 1 }, { upsert: true }).then(
          (qq) => {
...
app.get("/d1/:game_id", (req, res) => {
  ...
  if (active_game[0] == 1) {
    ...
    if (this_server_type == "tutorial" && active_game[0] == 0) {
      console.log(
        "game is being saved into db?? maybe??????????????????????????????????????"
      );
    } else {
