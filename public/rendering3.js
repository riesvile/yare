...
function offsetUpdate() {
  ...
  c_base.clearRect(
    -offsetX,
    -offsetY,
    main_canvas.width * multiplier,
    main_canvas.height * multiplier
  );
    ...
    if (hover_content.length == 0) {
      ...
      if (hover_content[0][0] == "spirit") {
        ...
        hoveroid.innerHTML =
          ...
          " · </span>" +
          hover_content[0][6] +
          "<span class='lowlight'> hp</span></span><span class='base_energy'>" +
          hover_content[0][2] +
          " <span class='lowlight'>energy</span></span>";
        hoveroid.innerHTML +=
...
document
  .getElementById("console_head")
  .addEventListener("click", console_toggle, false);
...
function create_spirit_p1(spir_id) {
  ...
  spirit_lookup[spir_id] = new Spirit(
    spir_id,
    spir_position,
    spir_size,
    spir_energy,
...
class Base {
  ...
  draw() {
    ...
    if (this.shape == "circles") {
      ...
      c.strokeStyle =
        ...
        ")";
      c.strokeRect(
        this.position[0] - outer_width / 2,
        this.position[1] - outer_width / 2,
        outer_width,
        outer_width
      );
      ...
      c.fillStyle =
        ...
        color_parts[2] +
        ", " +
        mapValues(production_percent, 0, 0.25, 0, 1) +
        ")";
      c.fillRect(
        this.position[0] - outer_width / 2 + 2,
        this.position[1] - outer_width / 2 - 2,
        outer_width,
        4
      );
      ...
      c.fillStyle =
        ...
        color_parts[2] +
        ", " +
        mapValues(production_percent, 0.5, 0.75, 0, 1) +
        ")";
      c.fillRect(
        this.position[0] - outer_width / 2 - 2,
        this.position[1] + outer_width / 2 - 2,
      ...
      c.fillStyle =
        ...
        color_parts[2] +
        ", " +
        mapValues(production_percent, 0.25, 0.5, 0, 1) +
        ")";
      c.fillRect(
        this.position[0] + outer_width / 2 - 2,
        this.position[1] - outer_width / 2 + 2,
      ...
      c.fillStyle =
        ...
        color_parts[2] +
        ", " +
        mapValues(production_percent, 0.75, 1, 0, 1) +
        ")";
      c.fillRect(
        this.position[0] - outer_width / 2 - 2,
        this.position[1] - outer_width / 2 - 2,
      ...
      let progress_color =
        ...
        color_parts[2] +
        ", " +
        color_parts[3] * production_percent +
        ")";
  ...
  charge(prev_energy, new_energy) {
    if (
      ...
      prev_energy - new_energy > this.current_spirit_cost / 1.5
    ) {
...
function render_state(timestamp) {
  ...
  for (i = 0; i < all_spirits.length; i++) {
    if (spirit_lookup[all_spirits[i]].player_id == pla1) {
      try {
        spirit_lookup[all_spirits[i]].move(
          [
        ...
        spirit_lookup[all_spirits[i]].energize(
        ...
        spirit_lookup[all_spirits[i]].size_change(
  ...
  for (i = 0; i < specials.length; i++) {
    if (
      specials[i][0] == "sh" &&
      spirit_lookup[specials[i][1]].player_id == pla1 &&
      ...
      specials[i][0] == "sh" &&
      spirit_lookup[specials[i][1]].player_id == pla2 &&
  ...
  bases[0].charge(
    game_blocks[active_block].b1[4],
    game_blocks[active_block].b1[0]
  );
  bases[1].charge(
    game_blocks[active_block].b2[4],
    game_blocks[active_block].b2[0]
  );
