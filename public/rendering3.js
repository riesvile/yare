...
function baseOffsetUpdate() {
  ...
  for (i = 0; i < world_bases; i++) {
    c_base.clearRect(
      base_lookup[bases[i].id].position[0] - 40,
      base_lookup[bases[i].id].position[1] - 40,
      base_lookup[bases[i].id].position[0] + 40,
      base_lookup[bases[i].id].position[0] + 40
    );
...
function offsetUpdate() {
  ...
  c_base.clearRect(
    -offsetX,
    -offsetY,
    main_canvas.width * multiplier * 1.1,
    main_canvas.height * multiplier * 1.1
  );
...
function draw_boxsand_bg() {
  ...
  c.fillRect(
    -offsetX,
    -offsetY,
    main_canvas.width * multiplier * 1.2,
    main_canvas.height * multiplier * 1.2
  );
}
...
function pinchMove(e) {
  ...
  scale = Math.round(Math.min(Math.max(0.5, scale), 2) * 100) / 100;
  multiplier = 1 / scale;
...
function onPointerDown(e) {
  ...
  if (scaling != true) {
    ...
    if (el_id == "panel_dragger") {
      ...
      dragging_timeline = 1;
      percent_current =
        ((e.clientX - timeline_el_bound.left) / timeline_el_bound.width) * 100;
...
function fill_hover_thing(xx, yy, board_xx, board_yy) {
  ...
  try {
    ...
    for (i = 0; i < living_spirits.length; i++) {
      ...
      if (
        Math.abs(living_spirits[i].position[0] - board_xx) <= 10 &&
        Math.abs(living_spirits[i].position[1] - board_yy) <= 10
      ) {
        hover_content.push([
          "spirit",
          living_spirits[i].id,
          Math.floor(living_spirits[i].energy),
        ]);
      }
    }
    ...
    for (b = 0; b < bases.length; b++) {
      if (
        Math.abs(bases[b].position[0] - board_xx) <= 30 &&
        Math.abs(bases[b].position[1] - board_yy) <= 30
      ) {
        hover_content.push([
          "base",
          bases[b].id,
          Math.floor(bases[b].energy),
          bases[b].position,
          bases[b].def_status,
          bases[b].current_spirit_cost,
          bases[b].control,
        ]);
      }
    }
    ...
    for (s = 0; s < stars.length; s++) {
      if (
        Math.abs(stars[s].position[0] - board_xx) <= 50 &&
        Math.abs(stars[s].position[1] - board_yy) <= 50
      ) {
    ...
    for (o = 0; o < outposts.length; o++) {
      if (
        Math.abs(outposts[o].position[0] - board_xx) <= 50 &&
        Math.abs(outposts[o].position[1] - board_yy) <= 50
      ) {
        hover_content.push([
          "outpost",
          outposts[o].control,
    ...
    for (p = 0; p < pylons.length; p++) {
      if (
        Math.abs(pylons[p].position[0] - board_xx) <= 50 &&
        Math.abs(pylons[p].position[1] - board_yy) <= 50
      ) {
    ...
    if (hover_content.length == 0) {
      ...
      if (hover_content[0][0] == "spirit") {
        hoveroid.innerHTML =
          "<span class='spirit_id'>" +
          hover_content[0][1] +
          "</span><span class='spirit_energy'>" +
          hover_content[0][2] +
          " <span class='lowlight'>energy</span></span>";
      } else if (hover_content[0][0] == "base") {
        let base_control = hover_content[0][6];
        if (base_control == "") {
          base_control = "neutral";
          hoveroid.innerHTML =
            "<span class='base_id'><span class='lowlight'>" +
            hover_content[0][1] +
            " · </span>" +
            base_control;
        } else {
          hoveroid.innerHTML =
            ...
            " · </span>" +
            hover_content[0][6] +
            "<span class='base_energy'>" +
            hover_content[0][2] +
            " <span class='lowlight'>energy</span></span>";
          hoveroid.innerHTML +=
            "<span class='new_when'>new spirit at <span class='highlight'>" +
            hover_content[0][5] +
            "</span></span>";
          if (hover_content[0][4] == 1) {
            hoveroid.innerHTML +=
              "<span class='under_attack'>enemies in sight, production paused</span>";
          }
        }
        ...
        hoveroid.innerHTML =
          "<span class='star_id'>" + hover_content[0][1] + "</span>";
        hoveroid.innerHTML +=
          "<span class='star_energy'>" +
          hover_content[0][2] +
          "<span class='lowlight'> energy</span></span>";
        hoveroid.style.bottom = window.innerHeight - yy + 10 + "px";
        ...
        hoveroid.innerHTML =
          "<span class='outpost_control'>outpost_mdo <span class='lowlight'> " +
          hover_content[0][1] +
          "</span></span>";
        hoveroid.innerHTML +=
          "<span class='outpost_energy'>" +
          hover_content[0][2] +
          "<span class='lowlight'> energy</span></span>";
        hoveroid.style.bottom = window.innerHeight - yy + 10 + "px";
        hoveroid.style.left = xx - 20 + "px";
        ...
        hoveroid.innerHTML =
          "<span class='pylon_control'>pylon_u3p <span class='lowlight'> " +
          hover_content[0][1] +
          "</span></span>";
        hoveroid.innerHTML +=
          "<span class='pylon_energy'>" +
          hover_content[0][2] +
          "<span class='lowlight'> energy</span></span>";
        hoveroid.style.bottom = window.innerHeight - yy + 10 + "px";
        hoveroid.style.left = xx - 20 + "px";
      ...
      hoveroid.innerHTML =
        ...
        hover_content[0][1] +
        " + " +
        (hover_content.length - 1) +
        " spirits</span><span class='spirit_energy'>" +
        total_eng +
        " <span class='lowlight'>total energy</span></span>";
    } else {
      show_hover();
      var temp_fill = "";
      for (j = 0; j < hover_content.length; j++) {
...
function game_over(winner) {
  ...
  fetch("/gameinfo", {
    ...
    .then((response) => {
      ...
      document.getElementById("player1_shape").innerHTML =
        "<span class='ico_" +
        shapes["shape1"] +
        "' style='background-color: " +
        colors["color1"] +
        "'></span>";
      document.getElementById("player2_shape").innerHTML =
        "<span class='ico_" +
        shapes["shape2"] +
        "' style='background-color: " +
        colors["color2"] +
      ...
      setTimeout(function () {
        fetch("/playerinfo", {
          ...
          .then((response) => {
            ...
            var p111_delta_string =
              "<span class='player_delta' id='player1_delta'>+" +
              p111_delta +
              "</span>";
            var p222_delta_string =
              "<span class='player_delta' id='player2_delta'>+" +
              p222_delta +
              "</span>";

            if (p111_delta > 0) {
              p111_delta_string =
                "<span class='player_delta delta_positive' id='player1_delta'>+" +
                p111_delta +
                "</span>";
            } else if (p111_delta < -1) {
              p111_delta_string =
                "<span class='player_delta delta_negative' id='player1_delta'>" +
                p111_delta +
                "</span>";
            }
            ...
            if (p222_delta > 0) {
              p222_delta_string =
                "<span class='player_delta delta_positive' id='player2_delta'>+" +
                p222_delta +
                "</span>";
            } else if (p222_delta < -1) {
              p222_delta_string =
                "<span class='player_delta delta_negative' id='player2_delta'>" +
                p222_delta +
                "</span>";
            }

            document.getElementById("player1_rating").innerHTML =
              p111_rating + p111_delta_string;
            document.getElementById("player2_rating").innerHTML =
              p222_rating + p222_delta_string;
          })
          .catch((err) => {
            console.log(err);
          });
  ...
  if (tutorial_phase > 1) {
    ...
    if (getCookie("user_id") == "anonymous") {
      document.getElementById("over_new_account").style.display =
        "inline-block";
...
function create_spirit_p1(spir_id) {
  var spir_position = [
    game_blocks["t" + tick_counter].p1[spir_id][0][0],
    game_blocks["t" + tick_counter].p1[spir_id][0][1],
  ];
...
function create_spirit_p2(spir_id) {
  ...
  var spir_position = [
    game_blocks["t" + tick_counter].p2[spir_id][0][0],
    game_blocks["t" + tick_counter].p2[spir_id][0][1],
  ];
...
class Spirit {
  ...
  energize(prev_energy, new_energy, hp) {
    ...
    if (this.hp != 0) {
      ...
      this.energy =
        prev_energy + (new_energy - prev_energy) * (total_time / game_tick);
  ...
  size_change(prev_size, new_size) {
    if (prev_size != new_size) {
      ...
      this.energy_capacity =
        prev_size * 10 + (new_size - prev_size) * 10 * (total_time / game_tick);
    ...
    if (this.hp != 0) {
      ...
      try {
        ...
        gradient.addColorStop(
          ...
          "rgba(" +
            ...
            color_parts[2] +
            ", " +
            (color_parts[3] * spirit_percent_energy) / 20 +
            ")"
        );
        gradient.addColorStop(
          1,
      ...
      if (this.shape == "circles") {
        ...
        c.strokeStyle =
          ...
          color_parts[2] +
          ", " +
          color_parts[3] * (spirit_percent_energy / 2 + 0.8) +
          ")";
        //if (this.hp > 0){
        c.stroke();
        //}
        ...
        c.arc(
          this.position[0],
          this.position[1],
          drawing_size * spirit_percent_energy,
          0,
          Math.PI * 2,
          false
        );
        c.fillStyle =
          ...
          color_parts[2] +
          ", " +
          color_parts[3] * (spirit_percent_energy / 2 + 0.5) +
          ")";
        c.fill();
        ...
        c.fillStyle =
          ...
          ")";
        c.fillRect(
          this.position[0] - (this.size * spirit_percent_energy) / 2,
          this.position[1] - (this.size * spirit_percent_energy) / 2,
          this.size * spirit_percent_energy,
          this.size * spirit_percent_energy
        );
        ...
        draw_polygon(
          ...
          "rgba(" +
            ...
            color_parts[2] +
            ", " +
            color_parts[3] * (spirit_percent_energy / 2 + 0.6) +
            ")",
          0,
          this.tria
        );
        ...
        draw_polygon(
          ...
          this.position[1],
          3,
          (this.size + 1) * spirit_percent_energy,
          1,
          0,
          current_color,
          this.tria
      ...
      if (this.shout != "") {
        ...
        c.fillStyle =
          ...
          color_parts[2] * 1.4 +
          ", " +
          color_parts[3] * (shouting_helper[this.id] / 180) +
          ")";
        c.textAlign = "center";
...
class Star {
  ...
  draw() {
    ...
    c_base.shadowBlur =
      this.size / ((4 + this.size / 130) * (multiplier / 2.5));
    c_base.lineWidth = 8;
    c_base.stroke();
    //c_base.stroke();
...
function mapValues(the_number, in_min, in_max, out_min, out_max) {
  return (
    ((the_number - in_min) * (out_max - out_min)) / (in_max - in_min) + out_min
  );
}
...
class Base {
  constructor(id, position, energy, player, color, shape, def_status = 0) {
    ...
    structure_lookup[this.id] = this;
    if (this.shape == "triangles")
      this.base_points = get_triangle(this.position[0], this.position[1], 30);
  }
  ...
  draw(cntrl = "") {
    ...
    if (this.shape == "circles") {
      ...
      c.strokeStyle =
        ...
        color_parts[2] +
        ", " +
        mapValues(this.def_status, 0, 1, 0.69, 1) +
        ")";
      c.lineWidth = 10;
      c.stroke();
      c.setLineDash([]);
      ...
      c.strokeStyle =
        ...
        color_parts[2] +
        ", " +
        mapValues(this.def_status, 0, 1, 0, 1) +
        ")";
      ...
      c.strokeStyle =
        ...
        ")";
      c.strokeRect(
        this.position[0] - 4 - this.size / 2,
        this.position[1] - 4 - this.size / 2,
        this.size + 8,
        this.size + 8
      );
      ...
      if (stroke_width > 1) {
        ...
        c.strokeStyle =
          ...
          ")";
        c.strokeRect(
          this.position[0] - stroke_offset - this.size / 2,
          this.position[1] - stroke_offset - this.size / 2,
          this.size + stroke_offset * 2,
          this.size + stroke_offset * 2
        );
      }
    } else if (this.shape == "triangles") {
      ...
      let R = (L * 0.5) / Math.cos(Math.PI / 6);
      ...
      let tri_defense_color =
        ...
        color_parts[2] +
        ", " +
        mapValues(this.def_status, 0, 1, 0, 0.5) +
        ")";
      ...
      c.roundRect(
        this.position[0] - 9 - this.size / 2,
        this.position[1] - 9 - this.size / 2,
        this.size + 18,
        this.size + 18,
        16
      );
      c.stroke();
      c.roundRect(
        this.position[0] - 2 - this.size / 2,
        this.position[1] - 2 - this.size / 2,
        this.size + 4,
        this.size + 4,
        10
      );
  ...
  defend(new_status) {
    ...
    if (new_status != this.def_status) {
      this.def_status = Math.abs(
        new_status * (total_time / game_tick) +
          (new_status - 1) * (total_time / game_tick - 1)
      );
      if (Math.abs(this.def_status - new_status) < 0.05)
        this.def_status = new_status;
...
class Outpost {
  ...
  draw(enrg, cntrl = "") {
    ...
    if (cntrl != "") {
      ...
      let range_gradient = c.createRadialGradient(
        this.position[0],
        this.position[1],
        200,
        this.position[0],
...
function draw_energize(origin, target, energy_strength, color) {
  ...
  try {
    ...
    grad.addColorStop(
      0.06,
      "rgba(" +
        (Number(color_parts[0]) + 80) +
        ", " +
        (Number(color_parts[1]) + 50) +
        ", " +
        (Number(color_parts[2]) + 50) +
        ", " +
        color_parts[3] +
        ")"
    );
...
function initiate_world() {
  ...
  for (i = 0; i < world_bases; i++) {
    base_lookup[bases_queue[i].id] = new Base(
      bases_queue[i].id,
      bases_queue[i].position,
      bases_queue[i].energy,
      bases_queue[i].player_id,
      bases_queue[i].color,
      bases_queue[i].shape
    );
  ...
  corner1_parts_hsl = rgb_to_hsl(
    corner1_parts[0],
    corner1_parts[1],
    corner1_parts[2]
  );
  corner2_parts_hsl = rgb_to_hsl(
    corner2_parts[0],
    corner2_parts[1],
    corner2_parts[2]
  );
...
function render_state(timestamp) {
  ...
  try {
    for (i = 0; i < all_spirits.length; i++) {
      if (spirits[all_spirits[i]].player_id == players["player1"]) {
        try {
          spirits[all_spirits[i]].move(
            [
              game_blocks[active_block].p1[all_spirits[i]][0][4],
              game_blocks[active_block].p1[all_spirits[i]][0][5],
            ],
            [
              game_blocks[active_block].p1[all_spirits[i]][0][2],
              game_blocks[active_block].p1[all_spirits[i]][0][3],
            ]
          );
          spirits[all_spirits[i]].energize(
            game_blocks[active_block].p1[all_spirits[i]][5],
            game_blocks[active_block].p1[all_spirits[i]][2],
            game_blocks[active_block].p1[all_spirits[i]][3]
          );
          spirits[all_spirits[i]].size_change(
            game_blocks[active_block].p1[all_spirits[i]][4],
            game_blocks[active_block].p1[all_spirits[i]][1]
          );
        } catch (e) {
          console.log(e);
        }
        ...
        try {
          spirits[all_spirits[i]].move(
            [
              game_blocks[active_block].p2[all_spirits[i]][0][4],
              game_blocks[active_block].p2[all_spirits[i]][0][5],
            ],
            [
              game_blocks[active_block].p2[all_spirits[i]][0][2],
              game_blocks[active_block].p2[all_spirits[i]][0][3],
            ]
          );
          spirits[all_spirits[i]].energize(
            game_blocks[active_block].p2[all_spirits[i]][5],
            game_blocks[active_block].p2[all_spirits[i]][2],
            game_blocks[active_block].p2[all_spirits[i]][3]
          );
          spirits[all_spirits[i]].size_change(
            game_blocks[active_block].p2[all_spirits[i]][4],
            game_blocks[active_block].p2[all_spirits[i]][1]
          );
        } catch (e) {
          console.log(e);
        }
  ...
  try {
    ...
    for (i = 0; i < specials.length; i++) {
      if (
        specials[i][0] == "sh" &&
        spirits[specials[i][1]].player_id == pla1 &&
        shouting_count_p1 < 100
      ) {
        ...
        specials[i][0] == "sh" &&
        spirits[specials[i][1]].player_id == pla2 &&
        shouting_count_p2 < 100
      ) {
        if (shouting[specials[i][1]] == null || shouting[specials[i][1]] == 0) {
          shouting[specials[i][1]] = 3;
          handle_shout(specials[i][1], specials[i][2]);
  ...
  for (let b = 0; b < bases.length; b++) {
    ...
    if (game_blocks[active_block]["b" + (b + 1)][2] > 0)
      bases_def_status[b] = 1;
    bases[b].charge(
      game_blocks[active_block]["b" + (b + 1)][4],
      game_blocks[active_block]["b" + (b + 1)][0]
    );
  ...
  outposts[0].draw(
    game_blocks[active_block].ou[0],
    game_blocks[active_block].ou[1]
  );
  pylons[0].draw(
    game_blocks[active_block].py[0],
    game_blocks[active_block].py[1]
  );
  ...
  for (let f = 0; f < game_blocks[active_block].ef.length; f++) {
    ...
    let frag_grd = c.createRadialGradient(
      fragment[0][0],
      fragment[0][1],
      1,
      fragment[0][0],
    ...
    frag_grd.addColorStop(
      1,
      "rgba(248, 247, 255, " +
        (0.05 + Math.min((0.1 * fragment[1]) / 100, 0.1) + fragment[1] / 1200) +
        ")"
    );
...
(function () {
  ...
  if (!window._move) window._move = Spirit.prototype.move;
  ...
  Spirit.prototype.move = function () {
    ...
    if (!this.noiseB) {
      // move this to the constructor instead of checking if it exists
