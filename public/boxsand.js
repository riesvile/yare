...
function render_world(dat) {
  ...
  if (tick_counter > 1) {
    ...
    for (i = 0; i < incoming.p1.length; i++) {
      ...
      game_blocks["t" + tick_counter].units.push(
        incoming.pl1 + "_" + incoming.p1[i][0]
      );
    ...
    for (j = 0; j < incoming.p2.length; j++) {
      ...
      game_blocks["t" + tick_counter].units.push(
        incoming.pl2 + "_" + incoming.p2[j][0]
      );
    ...
    game_blocks["t" + tick_counter]["b3"][4] =
      game_blocks["t" + (tick_counter - 1)]["b3"][0];
    game_blocks["t" + tick_counter]["b4"][4] =
      game_blocks["t" + (tick_counter - 1)]["b4"][0];
...
function set_stage() {
  ...
  if (players["player1"] == "live-input" && players["player2"] == "live-input")
    bs_error("You can control only one player with live input");
  ...
  if (boxsanded["p1_def"]) {
    ...
    for (let s = 0; s < boxsanded["p1_units"].length; s++) {
      ...
      game_blocks[active_block]["p1"][players["player1"] + "_" + spi[0]] = [
        spi[1],
        spi[2],
        spi[3],
        1,
  ...
  if (boxsanded["p2_def"]) {
    ...
    for (let q = 0; q < boxsanded["p2_units"].length; q++) {
      ...
      game_blocks[active_block]["p2"][players["player2"] + "_" + spi[0]] = [
        spi[1],
        spi[2],
        spi[3],
        1,
...
function update_game_shapes(pl2 = 1) {
  if (pl2 == 0) {
    for (let sp_id of Object.keys(spirits)) {
      ...
      game_blocks[active_block]["p1"][sp_id][1] = get_def_size(
        boxsanded["pl1_sh"]
      );
      game_blocks[active_block]["p1"][sp_id][2] =
        get_def_size(boxsanded["pl1_sh"]) * 10;
      game_blocks[active_block]["p1"][sp_id][4] = get_def_size(
        boxsanded["pl1_sh"]
      );
    ...
    for (let sp_id of Object.keys(spirits)) {
      ...
      game_blocks[active_block]["p2"][sp_id][1] = get_def_size(
        boxsanded["pl2_sh"]
      );
      game_blocks[active_block]["p2"][sp_id][2] =
        get_def_size(boxsanded["pl2_sh"]) * 10;
      game_blocks[active_block]["p2"][sp_id][4] = get_def_size(
        boxsanded["pl2_sh"]
      );
...
function fill_defaults(pl2 = 1) {
  ...
  if (pl2 == 0) {
    ...
    for (let s = 1; s < 1 + start_num_spirits - start_num_adjust1; s++) {
      if (s > 6) {
        game_blocks[active_block]["p1"][players["player1"] + "_" + s] = [
          [-620 - s * 20, -600],
          get_def_size(boxsanded["pl1_sh"]),
          get_def_size(boxsanded["pl1_sh"]) * 10,
          1,
          1,
          1,
        ];
        boxsanded["p1_units"].push([
          s,
          [-620 - s * 20, -600],
          get_def_size(boxsanded["pl1_sh"]),
          get_def_size(boxsanded["pl1_sh"]) * 10,
      ...
      let oldLoc = [0, 0];
      let newLoc =
        game_blocks[active_block].p1[players["player1"] + "_" + s][0];
      let oldEnergy =
        game_blocks[active_block].p1[players["player1"] + "_" + s][2];
      let oldSize =
        game_blocks[active_block].p1[players["player1"] + "_" + s][1];
      game_blocks[active_block].p1[players["player1"] + "_" + s][0][2] = 0;
      game_blocks[active_block].p1[players["player1"] + "_" + s][0][3] = 0;
      game_blocks[active_block].p1[players["player1"] + "_" + s][0][4] =
        newLoc[0];
      game_blocks[active_block].p1[players["player1"] + "_" + s][0][5] =
        newLoc[1];
    ...
    for (let q = 1; q < 1 + start_num_spirits - start_num_adjust2; q++) {
      if (q > 6) {
        game_blocks[active_block]["p2"][players["player2"] + "_" + q] = [
          [450 + q * 20, 770],
          get_def_size(boxsanded["pl2_sh"]),
          get_def_size(boxsanded["pl2_sh"]) * 10,
          1,
          1,
          1,
        ];
        boxsanded["p2_units"].push([
          q,
          [450 + q * 20, 770],
          get_def_size(boxsanded["pl2_sh"]),
          get_def_size(boxsanded["pl2_sh"]) * 10,
      ...
      let oldLoc = [0, 0];
      let newLoc =
        game_blocks[active_block].p2[players["player2"] + "_" + q][0];
      let oldEnergy =
        game_blocks[active_block].p2[players["player2"] + "_" + q][2];
      let oldSize =
        game_blocks[active_block].p2[players["player2"] + "_" + q][1];
      game_blocks[active_block].p2[players["player2"] + "_" + q][0][2] = 0;
      game_blocks[active_block].p2[players["player2"] + "_" + q][0][3] = 0;
      game_blocks[active_block].p2[players["player2"] + "_" + q][0][4] =
        newLoc[0];
      game_blocks[active_block].p2[players["player2"] + "_" + q][0][5] =
        newLoc[1];
...
function get_pick_state(pl) {
  ...
  if (boxsanded[pl] == "live-input") {
    document
      .getElementById("live_shape_" + live_bot_shape[pl])
      .parentNode.classList.add("ps_shape_active");
  } else if (boxsanded[pl] == "upload-bot") {
    document
      .getElementById("upload_shape_" + upload_bot_shape[pl])
      .parentNode.classList.add("ps_shape_active");
  }
...
function reset_live_shape_sel() {
  document
    .getElementById("live_shape_circles")
    .parentNode.classList.remove("ps_shape_active");
  document
    .getElementById("live_shape_squares")
    .parentNode.classList.remove("ps_shape_active");
  document
    .getElementById("live_shape_triangles")
    .parentNode.classList.remove("ps_shape_active");
}

function reset_upload_shape_sel() {
  document
    .getElementById("upload_shape_circles")
    .parentNode.classList.remove("ps_shape_active");
  document
    .getElementById("upload_shape_squares")
    .parentNode.classList.remove("ps_shape_active");
  document
    .getElementById("upload_shape_triangles")
    .parentNode.classList.remove("ps_shape_active");
}
...
function set_sel(pl, elname) {
  ...
  document.getElementById("bs_" + pl + "_name").innerHTML =
    "<span class='lowlight'>" + plnumtext + " · </span>" + el_option;
}
...
function place_element(loc) {
  ...
  game_blocks[active_block]["p" + pla_num][sp_id] = [
    loc,
    get_def_size(boxsanded["pl" + pla_num + "_sh"]),
    get_def_size(boxsanded["pl" + pla_num + "_sh"]) * 10,
    1,
    get_def_size(boxsanded["pl" + pla_num + "_sh"]),
    get_def_size(boxsanded["pl" + pla_num + "_sh"]) * 10,
  ];
  boxsanded["p" + pla_num + "_units"].push([
    spnum,
    loc,
    get_def_size(boxsanded["pl" + pla_num + "_sh"]),
    get_def_size(boxsanded["pl" + pla_num + "_sh"]) * 10,
  ...
  spirits[sp_id] = new Spirit(
    ...
    get_def_size(boxsanded["pl" + pla_num + "_sh"]),
    get_def_size(boxsanded["pl" + pla_num + "_sh"]) * 10,
    players["player" + pla_num],
    colors["color" + pla_num],
    boxsanded["pl" + pla_num + "_sh"],
    1
  );
...
function delete_element(loc) {
  ...
  for (let sp_id of Object.keys(all_sp1)) {
    ...
    if (max_abs_dist(sp.position, loc) <= 10) {
      game_blocks[active_block]["units"] = game_blocks[active_block][
        "units"
      ].filter((e) => e != sp_id);
      boxsanded["p1_units"] = boxsanded["p1_units"].filter(
        (e) => max_abs_dist(e[1], loc) > 10
      );
      living_spirits = living_spirits.filter(
        (e) => max_abs_dist(e.position, loc) > 10
      );
  ...
  for (let sp_id of Object.keys(all_sp2)) {
    ...
    if (max_abs_dist(sp.position, loc) <= 10) {
      ...
      boxsanded["p2_units"] = boxsanded["p2_units"].filter(
        (e) => max_abs_dist(e[1], loc) > 10
      );
      living_spirits = living_spirits.filter(
        (e) => max_abs_dist(e.position, loc) > 10
...
function tick_rate_select(e) {
  ...
  switch (el_id) {
    case "tick_05":
      reset_tick_ui();
      document.getElementById("tick_sec_desc").innerHTML =
        "1 tick = <span class='highlight'>1000ms</span>";
    ...
    case "tick_1":
      reset_tick_ui();
      document.getElementById("tick_sec_desc").innerHTML =
        "1 tick = <span class='highlight'>500ms</span>";
    ...
    case "tick_2":
      reset_tick_ui();
      document.getElementById("tick_sec_desc").innerHTML =
        "1 tick = <span class='highlight'>250ms</span>";
    ...
    case "tick_3":
      reset_tick_ui();
      document.getElementById("tick_sec_desc").innerHTML =
        "1 tick = <span class='highlight'>100ms</span>";
    ...
    case "tick_6":
      reset_tick_ui();
      document.getElementById("tick_sec_desc").innerHTML =
        "1 tick = <span class='highlight'>50ms</span>";
...
function console_window_toggle() {
  document
    .getElementById("console2_view_toggle")
    .classList.toggle("c2_view_open");
...
function console_view_all() {
  document
    .getElementById("c2_option_current")
    .classList.remove("c2_option_active");
