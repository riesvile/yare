  ...
  if (hover_content.length == 0) {
    ...
    if (hover_content[0][0] == "spirit") {
      ...
      hoveroid.innerHTML =
        ...
        hover_content[0][1] +
        "</span></span><span class='base_energy'>" +
        hover_content[0][2] +
        " <span class='lowlight'>energy · </span>" +
        hover_content[0][5] +
        "<span class='lowlight'> hp</span></span>";
  ...
  fetch("/gameinfo", {
    ...
    .then((response) => {
      ...
      document.getElementById("player1_shape").innerHTML =
        "<span class='ico_circle' style='background-color: " +
        colors["color1"] +
        "'></span>";
...
class Spirit {
  ...
  divide() {
    ...
    for (let d = 0; d < that.merged.length; d++) {
      ...
      spirit_lookup[that.merged[d]].energy = Math.floor(
        original_energy / original_size
      );
      spirit_lookup[that.merged[d]].energy_capacity =
        original_energy_capacity / original_size;
    }
...
class Base {
  ...
  charge() {
    ...
    c_base.arc(
      ...
      this.position[1],
      this.size + 10,
      Math.PI * 1.5 + new_angle,
      Math.PI * 2 * production_percent,
      false
    );
    console.log("production_percent");
    console.log(production_percent);
...
function initiate_world() {
  ...
  for (i = 0; i < world_spirits; i++) {
    ...
    //console.log(units_queue[i]);
    spirit_lookup[units_queue[i].id] = new Spirit(
      units_queue[i].id,
      units_queue[i].position,
      units_queue[i].size,
      units_queue[i].energy,
      units_queue[i].player_id,
      units_queue[i].color,
      units_queue[i].merged,
      units_queue[i].hp
    );
  ...
  for (i = 0; i < world_stars; i++) {
    ...
    star_lookup[stars_queue[i].id] = new Star(
      stars_queue[i].id,
      stars_queue[i].position,
      stars_queue[i].size
    );
...
function render_state(timestamp) {
  ...
  for (i = 0; i < birthlings; i++) {
    spirit_lookup[birth_queue[i].id] = new Spirit(
      birth_queue[i].id,
      birth_queue[i].position,
      birth_queue[i].size,
      birth_queue[i].energy,
      birth_queue[i].player_id,
      birth_queue[i].color,
      birth_queue[i].merged,
      birth_queue[i].hp
    );
    ...
    for (j = 0; j < bases.length; j++) {
      if (bases[j].player_id == birth_queue[i].player_id)
        bases[j].energy -= birth_queue[i].cost;
  ...
  for (i = 0; i < moveables; i++) {
    ...
    //console.log(move_queue[i]);
    try {
      spirit_lookup[move_queue[i][0]].move(
        move_queue[i][1],
        move_queue[i][2],
        move_queue[i][3]
      );
      ...
      spirit_lookup[move_queue[i][0]] = new Spirit(
        move_queue[i][0],
        move_queue[i][1],
        1,
        10,
    ...
    if (energize_queue[i][0].startsWith("star")) {
      try {
        if (spirit_lookup[energize_queue[i][1]].hp != 0) {
          draw_energize(
            star_lookup[energize_queue[i][0]].position,
            spirit_lookup[energize_queue[i][1]].position,
            energize_queue[i][2],
            spirit_lookup[energize_queue[i][1]].color
          );
          if (
            energy_processed[energize_queue[i][1]] != 1 &&
            spirit_lookup[energize_queue[i][1]].energy <=
              spirit_lookup[energize_queue[i][1]].energy_capacity
          ) {
      ...
      try {
        draw_energize(
          spirit_lookup[energize_queue[i][0]].position,
          base_lookup[energize_queue[i][1]].position,
          energize_queue[i][2],
          spirit_lookup[energize_queue[i][0]].color
        );
        if (energy_processed[energize_queue[i][0]] != 1) {
          if (
            spirit_lookup[energize_queue[i][0]].player_id !=
            base_lookup[energize_queue[i][1]].player_id
          ) {
            spirit_lookup[energize_queue[i][0]].energy -=
              energize_queue[i][2] / 2;
            ...
            if (spirit_lookup[energize_queue[i][0]].energy < 0)
              spirit_lookup[energize_queue[i][0]].energy = 0;
            base_lookup[energize_queue[i][1]].energy += energize_queue[i][2];
            //base_lookup[energize_queue[i][1]].draw();
            energy_processed[energize_queue[i][0]] = 1;
            //console.log('base energy');
      ...
      try {
        if (
          spirit_lookup[energize_queue[i][0]].hp != 0 &&
          spirit_lookup[energize_queue[i][1]].hp != 0
        ) {
          draw_energize(
            spirit_lookup[energize_queue[i][0]].position,
            spirit_lookup[energize_queue[i][1]].position,
          ...
          if (energy_processed[energize_queue[i][0]] != 1) {
            if (
              spirit_lookup[energize_queue[i][0]].player_id !=
              spirit_lookup[energize_queue[i][1]].player_id
            ) {
              spirit_lookup[energize_queue[i][0]].energy -=
                energize_queue[i][2] / 2;
              spirit_lookup[energize_queue[i][1]].energy -=
