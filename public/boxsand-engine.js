...
class Spirit {
  ...
  move(target) {
    ...
    if (isNaN(tarX) || isNaN(tarY)) {
      soft_error(
        ".move() arguments must be numbers, got [" + tarX + ", " + tarY + "]"
      );
      return;
    }
  ...
  energize(target) {
    ...
    if (bad) {
      ...
      soft_error(
        ".energize() argument must be a game object (e.g. spirit) with id or a position (e.g. [250, 350]).\n > E.g. my_spirits[0].energize(my_spirits[0])\n > or my_spirits[0].energize(spirits['" +
          example_id +
          "'])\n > or my_spirits[0].energize('" +
          example_id +
          "')\n > Received: " +
          target
      );
  ...
  merge(target) {
    if (target.id == this.id) {
      ...
      soft_error(
        "Only circles can use merge(). See Documentation for available methods."
      );
      return;
    }
    ...
    try {
      if (Array.isArray(target) == true) {
        soft_error(
          ".merge() argument must be a friendly spirit object, not an array. E.g. my_spirits[0].merge(my_spirits[1]). Received: " +
            target
        );
        return;
      } else if (typeof target !== "object" || target === null) {
        soft_error(
          ".merge() argument must be a friendly spirit object. E.g. my_spirits[0].merge(my_spirits[1]). Received: " +
            target
        );
        return;
      }
      ...
      if (
        Math.abs(target.position[0] - this.position[0]) < 12 &&
        Math.abs(target.position[1] - this.position[1]) < 12 &&
        this.player_id == target.player_id
      ) {
      } else {
        return;
      }
  ...
  divide() {
    if (this.shape != "circles") {
      soft_error(
        "Only circles can use divide(). See Documentation for available methods."
      );
      return;
    }
  ...
  jump(target) {
    if (Array.isArray(target) == false) {
      soft_error(
        ".jump() argument must be an array. E.g. my_spirits[0].jump([100, 100]). Received: " +
          target
      );
      return;
    } else if (target.length != 2) {
      soft_error(
        ".jump() argument must be an array of length 2. E.g. my_spirits[0].jump([100, 100]). Received: " +
          target
      );
      return;
  ...
  explode() {
    if (this.shape != "triangles") {
      soft_error(
        "Only triangles can use explode(). See Documentation for available methods."
      );
      return;
    }
  ...
  lock() {
    if (this.shape != "squares") {
      soft_error(
        "Only squares can use lock(). See Documentation for available methods."
      );
      return;
    }
    if (this.hp != 0) {
  ...
  unlock() {
    if (this.shape != "squares") {
      soft_error(
        "Only squares can use unlock(). See Documentation for available methods."
      );
      return;
    }
    if (this.hp != 0) {
...
function process_stuff() {
  ...
  for (i = 0; i < bases.length; i++) {
    ...
    if (bs.control == players["p1"]) {
      top_s++;
      globalThis[players["p1"] + top_s] = new Spirit(
        players["p1"] + "_" + top_s,
    ...
    if (bs.control == players["p2"]) {
      top_q++;
      globalThis[players["p2"] + top_q] = new Spirit(
        players["p2"] + "_" + top_q,
        [bs.position[0] + x_axis * 40, bs.position[1] + y_axis * 40],
...
function initiate_world() {
  ...
  for (let sp = 0; sp < p1sp.length; sp++) {
    let sp_num = sp + 1;
    globalThis[players["p1"] + "_" + sp_num] = new Spirit(
      players["p1"] + "_" + sp_num,
      p1sp[sp][1],
      p1sp[sp][2],
      p1sp[sp][3],
  ...
  for (let sp = 0; sp < p2sp.length; sp++) {
    let sp_num = sp + 1;
    globalThis[players["p2"] + "_" + sp_num] = new Spirit(
      players["p2"] + "_" + sp_num,
      p2sp[sp][1],
      p2sp[sp][2],
      p2sp[sp][3],
...
function run_code() {
  ...
  try {
    Function(player_codes["pl1_code"] + "memory1 = memory;")(
      (memory = memory1),
      (spirits = rawSpirits1),
      (my_spirits = my_spirits1),
      (this_player_id = players["p1"]),
      (ttick = "t" + tick)
    );
  ...
  try {
    Function(player_codes["pl2_code"] + "memory2 = memory;")(
      (memory = memory2),
      (spirits = rawSpirits2),
      (my_spirits = my_spirits2),
      (this_player_id = players["p2"]),
      (ttick = "t" + tick)
    );
...
function get_bot_code(botname, sour = "local", pl = "pl1") {
  if (sour == "local") {
    try {
      fetch("./bots/" + botname + ".js").then(function (response) {
        if (!response.ok) {
