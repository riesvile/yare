  ...
  knockknock = setInterval(function () {
    ...
    fetch("/check-status/" + g_id, {
      ...
      .then((response) => {
        ...
        if (link_filled == 0) {
          link_filled = 1;
          document.getElementById("ch_link").innerHTML =
            "https://yare.io/challenge/" + g_id;
          document
            .getElementById("friend_link")
            .setAttribute("code", "https://yare.io/challenge/" + g_id);
        }
...
pub_modules["mod_basic-info-graphs"] = {
  ...
  author: "yare.io",
  description:
    "Graph showing the difference between players' current energy and total energy levels. Clicking on the graph minimizes it.",
  type: "data viz",
...
pub_modules["mod_manual-controls"] = {
  ...
  author: "yare.io",
  description:
    "Play yare with your mouse like a traditional Real-Time Strategy.",
  type: "UI controls",
  client_script_location: "local",
...
function create_module() {
  ...
  if (has_client == 0 && has_server == 0) {
    ...
    document.getElementById("modules_server_message").innerHTML =
      "Upload at least one .js file";
    return;
...
function integrate_new_module(mod_id) {
  ...
  let user_module_html_string =
    ...
    "' id='mdl_" +
    m_id +
    "'><div class='module_card_bg' id='bg_mdl_mine'></div><div class='module_options' id='options_" +
    m_id +
    "'><a href='#' class='module_options_btn btn_edit' id='edit_" +
    m_id +
    "'>Edit</a><a href='#' class='module_options_btn btn_pre_delete_module' id='predel_" +
    m_id +
    "'>Delete</a><div class='delete_confirm' id='confirm_" +
    m_id +
    "'><a href='#' class='module_options_btn delete_module' id='del_" +
    m_id +
    "'>Yes, delete</a><a href='#' class='module_options_btn cancel_delete' id='cancel_" +
    m_id +
    "'>Cancel</a></div></div><div class='module_toggle' id='toggle_" +
    m_id +
    "'><div class='the_toggle' id='thetog'></div></div><h3 class='module_name'>" +
    script_name +
    "</h3></div>";

  document
    .getElementById("modules_section_mine")
    .insertAdjacentHTML("beforeend", user_module_html_string);

  let script_insert = document.createElement("script");
  script_insert.src =
    "https://yare.sfo3.digitaloceanspaces.com/modules/client/" + mod_id + ".js";
  document.head.appendChild(script_insert);
  console.log("module " + mod_id + " should be appended");

  document
    .getElementById("toggle_" + m_id)
    .addEventListener("click", toggle_toggle, false);
  document
    .getElementById("options_" + m_id)
    .addEventListener("click", mod_options_cross, false);
}
...
function update_module_info(module_id, delete_module = 0) {
  ...
  if (retreived_parsed == undefined || module_name == retreived_parsed["name"])
    module_name = "";
...
function upload_script(module_id, script_type, meta = "") {
  ...
  getBase64(
    document.getElementById("file_script_" + script_type).files[0]
  ).then((data) => {
    console.log(data);
    ...
    fetch("/upload-script", {
      ...
      .then((response) => {
        ...
        if (response.data == "script uploaded") {
          ...
          if (meta == "") {
            document.getElementById("modules_server_message").innerHTML =
              "Module updated. Check browser console for client script errors.";
          } else {
            document.getElementById("modules_server_message").innerHTML =
              "Module updated. Refresh the page to clear browser context";
          }
        }
      })
      .catch((err) => {
...
function local_server_script(module_id) {
  ...
  try {
    fetch(from_gameserver + "public-modules/server/" + module_id + ".js").then(
      function (response) {
    ...
    .then((response) => {
      ...
      if (response.data == "modules retreived") {
        ...
        for (let i = 0; i < response.stream.length; i++) {
          modules_local["mod_" + response.stream[i].module_id] =
            response.stream[i];
          modules_local["mod_" + response.stream[i].module_id]["active"] = 0;
          sessionStorage.setItem(
            "mod_" + response.stream[i].module_id,
            JSON.stringify(modules_local["mod_" + response.stream[i].module_id])
          );
        }
...
function toggle_toggle(e) {
  ...
  if (
    !el_parent.classList.contains("module_card") &&
    !el_parent.classList.contains("module_card_mine")
  )
  ...
  if (el_parent.classList.contains("module_off")) {
    ...
    try {
      ...
      sessionStorage.setItem(
        "mod_" + m_id,
        JSON.stringify(modules_local["mod_" + m_id])
      );
    } catch (e) {
      console.log(e);
    }
    ...
    .then((response) => {
      ...
      if (response.data == "modules retreived") {
        ...
        for (let i = 0; i < response.active_modules.length; i++) {
          ...
          sessionStorage.setItem(
            "mod_" + response.active_modules[i],
            JSON.stringify(modules_local["mod_" + response.active_modules[i]])
          );
        }
        integrate_modules();
      }
...
function activate_module(module_id) {
  ...
  sessionStorage.setItem(
    "mod_" + module_id,
    JSON.stringify(modules_local["mod_" + module_id])
  );
}
...
function set_active_modules() {
  ...
  for (const mod_id of Object.keys(modules_local)) {
    ...
    if (modules_local[mod_id]["active"] == 1)
      active_array.push(modules_local[mod_id]["module_id"]);
...
function integrate_modules() {
    ...
    for (let i = 0; i < card_insertion.length; i++) {
      ...
      if (m_author == user_name) {
        let user_module_html_string =
          ...
          "' id='mdl_" +
          m_id +
          "'><div class='module_card_bg' id='bg_mdl_mine'></div><div class='module_options'><a href='#' class='module_options_btn btn_edit' id='edit_" +
          m_id +
          "'>Edit</a><a href='#' class='module_options_btn btn_pre_delete_module' id='predel_" +
          m_id +
          "'>Delete</a><div class='delete_confirm' id='confirm_" +
        ...
        //public modules
        let public_module_html_string =
          "<div class='module_card " +
          mod_state +
          "' id='mdl_" +
          m_id +
          "'> <div class='module_card_bg' id='bg_mdl_" +
          m_id +
          "'></div> <span class='module_main_tag'>" +
          card_insertion[i].type +
          "</span> <div class='module_toggle' id='toggle_" +
          m_id +
          "'> <div class='the_toggle' id='thetog'></div> </div> <h3 class='module_name'>" +
          script_name +
          "</h3> <div class='module_ilu ilu_" +
          m_id +
          "' style='background:url(" +
          from_gameserver +
          "public-modules/ilu/" +
          m_id +
          ".png); background-size: cover;'></div> <p class='module_desc " +
          desc_type +
          "'>" +
          mod_desc +
          "</p> </div>";
        document
          .getElementById("modules_section")
          .insertAdjacentHTML("beforeend", public_module_html_string);
      }
    }
  } catch (e) {
    console.log(e);
  ...
  let ready_insertion = Object.values(modules_local).filter(
    (item) => item.active == 1
  );
  ...
  setTimeout(function () {
    for (let i = 0; i < ready_insertion.length; i++) {
      ...
      if (ready_insertion[i].public == 1) {
        script_insert.src =
          from_gameserver +
          "public-modules/client/" +
          ready_insertion[i].module_id +
          ".js";
        local_server_script(ready_insertion[i].module_id);
      } else {
        script_insert.src =
          "https://yare.sfo3.digitaloceanspaces.com/modules/client/" +
          ready_insertion[i].module_id +
          ".js";
      ...
      fetch("/add-user", {
        ...
        .then((response) => {
          ...
          if (response.data == "user created") {
            ...
            username_error(
              "Must be < 20 characters (yours is " +
                response.data2 +
                " characters)"
            );
            resume_client();
...
document
  .getElementById("profile_links")
  .addEventListener("click", settings_crossroad, false);
...
try {
  document
    .getElementById("create_new_module")
    .addEventListener("click", module_edit_mode_on, false);
  document
    .getElementById("close_module_edit")
    .addEventListener("click", module_edit_mode_off, false);
  document
    .getElementById("create_module_btn")
    .addEventListener("click", create_module, false);
  document
    .getElementById("update_module_btn")
    .addEventListener("click", update_module_info, false);

  document
    .getElementById("language_settings")
    .addEventListener("click", lang_toggle, false);
} catch (e) {}
