var bot_code = true;
if (memory["time"] == undefined) memory["time"] = 0;
memory["time"] += 1;
...
if (
  dist_sq(star_a1c.position, base.position) <
  dist_sq(star_zxq.position, base.position)
) {
  my_star = star_a1c;
  e_star = star_zxq;
}
...
function minimizing_unit(l, objective, selector) {
  ...
  for (var i = 0; i < l.length; i++) {
    ...
    if (selector != null && !selector(unit)) continue;
...
function closest_unit(ref, l, min_dist, pred) {
  ...
  for (var i = 0; i < l.length; i++) {
    ...
    if (min_dist > 0 && ud >= min_dist ** 2) continue;

    if (pred != null && !pred(unit)) continue;
...
function beam_from_to(from, to, shorten = 1) {
  return mult(
    shorten / dist(from.position, to.position),
    add(mult(-1, from.position), to.position)
  );
}
...
var h0_pos = add(
  my_star.position,
  beam_from_to(my_star, base, 0.99 * min_beam)
);
...
var my_alive = [];
var team_counts = teams.map(function (e) {
  return 0;
});
var no_teams = [];
...
for (var i = 0; i < my_spirits.length; i++) {
  ...
  if (!no_team && alive) {
    if (team_counts[t] >= teams[t] || t >= team_counts.length) {
      memory[s.id + "team"] = null;
      no_teams.push(s.id);
    } else team_counts[t] = team_counts[t] + 1;
  }
  if (alive) {
    my_alive.push(s.id);
  }
...
for (var i = 0; i < no_teams.length; i++) {
  for (var t = 0; t < team_counts.length; t++) {
    if (
      team_counts[t] < teams[t] &&
      (attack_groups.indexOf(t) == -1 || plans[t] == default_plan)
    ) {
...
if (bot_code) {
  for (var i = 0; i < attack_groups.length; i++) {
    ...
    if (
      memory["time"] > 50 &&
      (team_counts[idx] == teams[idx] || team_counts[idx] > 15) &&
      (plans[idx] == default_plan || long_ago)
    ) {
      var rnd = Math.random();
      if (rnd < 0.66 && !long_ago) plans[idx] = "S";
      else plans[idx] = "a";
...
for (var i = 0; i < my_alive.length; i++) {
  ...
  if (plan == "H" || (plan == "B" && base.sight.enemies.length == 0)) {
    ...
    if (memory[s.id] != "charged" && memory[s.id] != "harvestor") {
      if (d2base > d2star) memory[s.id] = "harvestor";
      else memory[s.id] = "charged";
    }
    ...
    if (e != null) {
      ...
      if (d2star > min_beam_sq) s.move(my_star.position);
      if (d2star < 0.8 * min_beam_sq) s.move(base.position);
    } else if (memory[s.id] == "charged") {
      if (d2base < 0.8 * min_beam_sq) s.move(my_star.position);
      if (d2base > min_beam_sq) s.move(base.position);
      s.energize(base);
    }
  } else {
    // state transitions
    ...
    if (memory[s.id] == "recharging") {
      if (d2star < d2estar) s.move(my_star.position);
      else s.move(e_star.position);
    }
    ...
    if (memory[s.id] == "fighter") {
      ...
      if (plan == "a") {
        ...
        if (!near_enemy) s.move(enemy_base.position);
        else s.energize(enemy_base);
      }

      if (plan == "A") {
        if (!near_estar) s.move(e_star.position);
      }
      ...
      if (plan == "n" || plan == "s") {
        ...
        if (move != null && move != undefined) {
          if (move.length > 2) {
            var which =
              Math.floor(memory["time"] / move[0]) % (move.length - 1);
  ...
  if (s.energy > 0) {
    ...
    e = closest_unit(s, s.sight.enemies_beamable, 0, (e) =>
      pick_enemy(e, seek)
    );
