...
var h0_pos = add(
  my_star.position,
  beam_from_to(my_star, my_base, 0.99 * min_beam)
);
...
for (var i = 0; i < my_alive.length; i++) {
  ...
  if (plan == "H" || (plan == "B" && my_base.sight.enemies.length == 0)) {
    ...
    if (e != null) {
      ...
      if (d2star < 0.8 * min_beam_sq) s.move(my_base.position);
    } else if (memory[s.id] == "charged") {
      if (d2base < 0.8 * min_beam_sq) s.move(my_star.position);
      if (d2base > min_beam_sq) s.move(my_base.position);
      s.energize(my_base);
    }
  } else {
    // state transitions
    ...
    if (memory[s.id] == "fighter") {
      ...
      if (plan == "a") {
        ...
        if (!near_enemy) s.move(e_base.position);
        else s.energize(e_base);
      }
