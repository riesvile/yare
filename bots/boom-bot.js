...
var defdist = outpost.energy > 300 ? -1 : -200;
...
var enemySize = Object.values(spirits)
  .filter((s) => s.hp > 0 && s.player_id != this_player_id)
...
var enemiesInRange = buildList(
  (s) => s.player_id != this_player_id,
  (s) => dist2(s.position, basePos)
);
...
for (var e of enemiesInRange) {
  ...
  glob = {
    position: mul(
      glob.reduce(
        (p, a) => {
          return add(p, spirits[a].position);
        },
        [0, 0]
      ),
      1 / glob.length
  ...
  all_globs.push(glob);
  var m = Math.sqrt(
    glob.members.reduce((p, a) => {
      return Math.max(p, dist2(glob.position, spirits[a].position));
    }, 400)
  );
...
if (outpost.control != this_player_id && tick > 200) {
  ...
  var en =
    outpost.energy / 2 +
    Object.values(spirits).reduce(
      (a, s) =>
        a +
        (s.hp > 0 &&
        s.player_id != this_player_id &&
        dist2(s.position, out_mid) < 400 * 400
          ? s.energy
          : 0),
      0
    );
  ...
  globs.push({
    ...
    energy:
      Object.values(spirits)
        .filter((s) => s.hp > 0 && s.player_id != this_player_id)
        .reduce((a, s) => a + s.energy_capacity, 0) *
        1 +
      enemy_base.energy * 1,
  });
}
...
for (var g of globs) {
  g.units = [];
  var energyGotten = 0;
  for (var h of buildList(
    (s) => s.player_id == this_player_id && !chosen[s.id],
    (s) =>
      dist2(s.position, g.position) + (s.energy_capacity - s.energy) * 20 * 20
  )) {
    chosen[h.s.id] = true;
    g.units.push(h.s);
...
var units = Math.round(
  Math.max(5, Math.min(maxUnits, mine.length - attackers))
);
...
for (var next of buildList(
  (s) => s.player_id == this_player_id && !chosen[s.id],
  (s) => dist2(s.position, relayPos)
).slice(0, relays)) {
...
var miners = buildList(
  (s) => s.player_id == this_player_id && !chosen[s.id],
  (s) => dist2(s.position, minePos)
).slice(0, miners);

var forwardminers = buildList(
  (s) =>
    s.player_id == this_player_id &&
    !chosen[s.id] &&
    dist2(s.position, basePos) < 375 * 375,
  (s) => dist2(s.position, basePos)
);
...
for (var g of toos) {
  ...
  if (
    energyGotten > g.minEnergy ||
    (memory.globs[g.id].attack && energyGotten > g.minEnergy * 0.5)
  ) {
    globs.push(g);
    memory.globs[g.id].attack = true;
    if (g.id == "outpost") {
...
healing = {};
carriers = my_spirits.reduce(
  (a, s) => a + (!chosen[s.id] && s.hp > 0.5 && s.carrier ? 1 : 0),
  0
);
...
if (outpost.control != this_player_id) {
  ...
  /*  HIDE OUTPOST HARVESTING*/
  var i = 0;
  out_mine = out_mine.reverse();
  var prior = [];
  for (var p of out_mine) {
    var units = buildListFromList(
      left,
      (s) => !chosen[s.id],
      (s) => dist2(s.position, p)
    )
      .slice(0, chains * (i == out_mine.length - 1 ? 2 : 1))
      .map((s) => s.s);
    for (var u of units) {
      chosen[u.id] = true;
      u.move(p);
      if (i == 0) {
        if (
          u.energy > u.energy_capacity / 2 &&
          dist2(u.position, base.position) < 200 ** 2
        ) {
          u.energize(base);
        }
        ...
        if (u.energy > u.energy_capacity / 2) {
          ...
          for (let n of prior) {
            if (
              n.energy < loweste &&
              dist2(u.position, n.position) < 200 ** 2
            ) {
              loweste = n.energy;
              lowest = n;
            }
        ...
        if (u.energy > u.energy_capacity / 2) {
          ...
          if (
            ...
            dist2(u.position, star_p89.position) < 200 ** 2
          ) {
            u.energize(u);
            outstargrowth -= u.size;
          }
...
for (var u of my_spirits) {
  ...
  var p = findClosest(
    u.sight.enemies_beamable,
    nextPos,
    (id) => spirits[id].energy - (e[id] || 0) < 0
  );
  if (p) {
    u.energize(p);
    energizing[u.id] = true;
    ...
    if (
      (outpost.control != this_player_id ||
        (u.energy > u.energy_capacity * 0.75 &&
          outpost.energy + e["outpost"] < outpost.energy_capacity)) &&
      dist2(u.position, outpost.position) < 200 * 200
    ) {
      energizing[u.id] = true;
      u.energize(outpost);
      e["outpost"] += u.size;
...
for (var g of globs) {
  ...
  for (var u of g.units) {
    ...
    if (u.energy == 0) {
      if (dist2(things.star.position, u.position) > 200 * 200) {
        if (
          dist2(u.position, things.star.position) <
            dist2(u.position, star_p89.position) ||
          star_p89.energy == 0
        ) {
  ...
  var blobAll = attackBlob.reduce(
    (p, a) => {
      return add(p, a.position);
    },
    [0, 0]
  );
  ...
  var m = g.units.reduce((p, a) => {
    return Math.max(p, dist2(blobCenter, a.position));
  }, 400);
  ...
  var allPassList = buildList(
    (s) => s.player_id == this_player_id,
    (s) => dist2(s.position, g.position)
  );
  var passList = buildList(
    (s) => inGlob[s.id],
    (s) => dist2(s.position, attackPos)
  );
