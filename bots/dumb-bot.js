...
if (
  memory["phase"] == 1 &&
  my_spirits[1].position[0] == 2600 &&
  my_spirits[5].position[0] == 2600
) {
  memory["phase"] = 2;
}
...
if (memory["phase"] == 2) {
  ...
  if (my_spirits[0].sight.enemies.length > 0) {
    ...
    if (
      Math.abs(enemy.position[0] - my_spirits[0].position[0]) < 250 &&
      Math.abs(enemy.position[1] - my_spirits[0].position[1]) < 250
    ) {
